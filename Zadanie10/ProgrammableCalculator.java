...
class ProgrammableCalculator implements ProgrammableCalculatorInterface {
    ...
    private Stack<Integer> savedLines = new Stack<>();
    ...
    public void run(int line) {
        ...
        while (iterator != null) {
            ...
            } else if (tempToUpper.contains("GOSUB")) {

                int goToLine = Integer.parseInt(currentLine.substring(currentLine.indexOf(" ") + 1));
                savedLines.push(iterator);
                iterator = goToLine;
                continue;

            } else if (tempToUpper.contains("RETURN")) {
                iterator = savedLines.pop();
            }
